...
public class SpringEmployeeMgmtAppClient {
	...
	private static void createEmployee() throws URISyntaxException {
		// Create Employee
		RequestEntity<Employee> requestEntity = new RequestEntity<Employee>(getEmployee(), HttpMethod.POST,
				new URI(BASE_URL + EMPLOYEES_SERVICE_URI));
		ResponseEntity<String> response = client.exchange(requestEntity, String.class);
		String message = response.getStatusCodeValue() == 201 ? "Employee Created Successfully"
	...
	private static void listEmployees() throws URISyntaxException {
		...
		RequestEntity<Employee> requestEntity = new RequestEntity<Employee>(HttpMethod.GET,
				new URI(BASE_URL + EMPLOYEES_SERVICE_URI));
